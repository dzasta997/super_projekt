...
public class ReportController {
    ...
    @GetMapping(value = "/transaction/{warehouseId}", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    public ResponseEntity<InputStreamResource> getTransactionReport(@PathVariable long warehouseId) throws IOException {
        MediaType contentType = MediaType.APPLICATION_PDF;
        InputStreamResource inputStreamResource = reportService.getTransactionReport(warehouseId);
        return ResponseEntity.ok().contentType(contentType).body(inputStreamResource);
    }
}
